...
def create_competences_per_employe(instance):
...
def create_competences_per_job(instance):
    """competences per job"""
    competences = dict()
...
def create_number_of_competences_per_job(instance):
    """competences per job"""
    competences = dict()
    for job in instance['jobs']:
...
def create_due_date_of_jobs(instance):
    """competences per job"""
    due_dates = dict()
    for job in instance['jobs']:
...
def create_gains_per_jobs(instance):
    """gains per job"""
    gains = dict()
    for job in instance['jobs']:
...
def create_delays_per_jobs(instance):
    """gains per job"""
...
def get_longest_project(instance):
    max_score = 0
...
def optimisation(instance,constr_longest_proj, constr_proj_per_employe):
    job_day = get_job_day(instance)

    long_proj = max(job_day, key=job_day.get)


    CP = working_days_per_qualification(instance)
    vacations = create_vacations(instance)
    ...
    project_max_duration = m.addVar(vtype=GRB.INTEGER, lb = 0, ub = horizon,  name='max_proj')
    ...
    competence_per_employe = create_competences_per_employe(instance) 
    competence_per_job = create_competences_per_job(instance) 
    ...
    number_of_competence_per_project = create_number_of_competences_per_job(instance)
    ...
    due_dates = create_due_date_of_jobs(instance)
    ...
    m.addConstr(project_max_duration == max_(nb_proj_per_employe))
    m.addConstr(project_max_duration == constr_proj_per_employe)
    m.addConstr(duration_max_project(get_longest_project(instance), E, S) == constr_longest_proj)
